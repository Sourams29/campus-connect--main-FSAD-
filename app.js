...
function isAuthenticated(req, res, next) {
  if (req.session.user) {
    console.log("printing user data",req.session.user,req.session);
...
function isAuthorized(roles) {
  return (req, res, next) => {
    // Check if the user is logged in and has one of the valid roles
    if (req.session.user && roles.includes(req.session.user.category)) {
      console.log("isAuthorized: User has access, going next.");
      return next();
    }
    // If user doesn't have a valid role or is not logged in
...
app.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (!email || !password || !category) {
            return res.redirect('/signup-form?error=All fields are required');
        }
        ...
        if (category === 'student') {
            const emailRegex = /^(\d{10})@mitwpu\.edu\.in$/;
            if (!emailRegex.test(email)) {
                return res.redirect('/signup-form?error=Invalid email format');
            }
            if (email.slice(0, 10) !== prn) {
                return res.redirect('/signup-form?error=PRN mismatch with email');
            }
        }

        // Check for duplicates
        const identifier = 
            category === 'student' ? prn :
            category === 'faculty' ? faculty_id :
            category === 'event_manager' ? event_id : null;

        const isDuplicate = await checkForDuplicate(identifier, email);
        if (isDuplicate) {
            return res.redirect('/signup-form?error=Credentials already in use');
        }

        // SQL Query Preparation
        let signupSql, values;
        if (category === 'student') {
            signupSql = 'INSERT INTO users (name, prn, email, password, category) VALUES (?, ?, ?, ?, ?)';
        ...
        res.redirect('/login'); // Successful signup redirects to login
    } catch (error) {
        console.error('Signup error:', error);
        res.redirect('/signup-form?error=Error during signup. Please try again.');
    }
});
...
const sendEmailToUsers = (eventData, users) => {
  const mailOptions = {
      from: 'aakarshsingh431@gmail.com',
      to: users.map(user => user.email).join(', '), // Send to all users
      subject: 'Event Date Changed',
      text: `Dear user,\n\nPlease note that the event "${eventData.name}" has had a change in its schedule. The new dates are:\n\nStart Date: ${eventData.start_date}\nEnd Date: ${eventData.end_date}\n\nThank you!`
  };
...
async function checkForDuplicate(identifier, email) {
  ...
  app.post('/faculty/sendMail', (req, res) => {
    const { eventId, message, facultyEmail } = req.body;
  
  
    if (!eventId || !message || !facultyEmail) {
        return res.status(400).json({ success: false, message: 'Event ID, message, and faculty email are required.' });
    }

    // Fetch the email addresses of registered users
    ...
    db.query(fetchEmailsSql, [eventId], (err, results) => {
        if (err) {
            console.error('Error fetching email addresses:', err);
            return res.status(500).json({ success: false, message: 'Error fetching email addresses.' });
        }

        if (results.length === 0) {
            return res.json({ success: false, message: 'No registered users found for this event.' });
        }
        ...
        const mailOptions = {
            ...
            subject: 'Message from Faculty Regarding Your Event',
            text : `
...
app.post('/admin/total-registrations', (req, res) => {
  const { eventId } = req.body;

  // Query to fetch event details and total registrations
  const fetchEventAndRegistrationsSql = `
      SELECT 
  ...
  db.query(fetchEventAndRegistrationsSql, [eventId], (err, results) => {
      if (err) {
      ...
      if (results.length === 0) {
          return res.json({ success: true, data: null, message: 'No event or registrations found for this event ID.' });
      }

      const eventData = results[0];
      console.log("Event and registration data fetched:", eventData);

      // Return the event details and total registrations
...
app.post('/login', (req, res) => {
  ...
  if (uname === 'admin@gmail.com' && psw === 'admin123') {
    ...
    return res.render('MainAdmin'); // Use return to prevent further execution
  }

  // Invalid admin credentials
  if (!uname || !psw) {
    return res.status(401).send('<script>alert("Invalid credentials! Please try again."); window.location.href="/login";</script>');
  }
  ...
  db.query(sql, [uname], async (err, results) => {
    if (err) {
      console.error(err);
      res.redirect('/login.html?error=notregistered');
    }
    ...
    case 'event_manager':
      const query = `
          SELECT event_id, name, category, registration_date, COUNT(DISTINCT name) AS total_registrations
          FROM registrations
          WHERE eventemail = ?
          GROUP BY event_id, name, category, registration_date;
      `;
      ...
      db.query(query, [uname], (err, results) => {
          ...
          const events = results.map(row => ({
              ...
              participant_name: row.name, // Use 'name' instead of 'event_name'
              category: row.category || "Not Available", // Fallback if category is missing
              registration_date: row.registration_date ? new Date(row.registration_date).toLocaleString() : "Not Available", // Format or fallback if invalid
...
app.post('/event', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const isDuplicate = await checkForDuplicateEvent(event.name);
    if (isDuplicate) {
      return res.status(400).json({ message: 'Event name is already taken. Please choose another.' });
    }
    ...
    db.query(
      ...
      (eventErr) => {
        if (eventErr) {
          console.error(eventErr);
          return res.status(500).json({ message: 'Error during event submission. Please try again.' });
        }
    ...
    console.error(error);
    res.status(500).json({ message: 'Error during event submission. Please try again.' });
  }
});
app.put('/admin/edit/:eventId', (req, res) => {
  ...
  // Fetch the event details by event_id (currentEventId)
  ...
  db.query(getEventQuery, [eventId], (err, results) => {
    ...
    // Proceed to update the event's start_date and end_date
    ...
    db.query(updateEventQuery, [start_date, end_date, eventId], (err, updateResults) => {
      if (err) {
      ...
      if (updateResults.affectedRows > 0) {
        // Fetch users' emails from the users table
        const getUsersEmailsQuery = 'SELECT email FROM users';
        db.query(getUsersEmailsQuery, (err, users) => {
          if (err) {
            console.error('Error fetching users:', err);
            return res.status(500).json({ message: 'Error fetching users' });
          }

          // Send email to all users after event update
          sendEmailToUsers(results[0], users);

          // Respond with success message
          return res.status(200).json({ message: 'Event updated successfully and emails sent to users' });
        });
      } else {
...
app.post('/registerEvent', (req, res) => {
  ...
  if (!name || !emailStudent || !category || !event_id || !emailEvent || !event_name || !purpose) {
    return res.render('register', {
      ...
      email: emailStudent,
      category,
      event: { id: event_id, name: event_name, email: emailEvent },
    });
  }

  const fetchEventSql = 'SELECT * FROM events WHERE id = ?';
  db.query(fetchEventSql, [event_id], (fetchErr, event) => {
    if (fetchErr || event.length === 0) {
      return res.render('register', {
        success: 0,
        message: 'Event not found or an error occurred.',
        name,
        email: emailStudent,
        category,
        event: { id: event_id, name: event_name, email: emailEvent },
    ...
    db.query(checkRegistrationSql, [emailStudent, event_id], (checkErr, existingRegistrations) => {
      ...
      const insertRegistrationSql =
        'INSERT INTO registrations (name, email, category, event_id, event_name, purpose, registration_date, eventemail) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)';
      db.query(insertRegistrationSql, [name, emailStudent, category, event_id, event_name, purpose, emailEvent], (err) => {
        if (err) {
          return res.render('register', {
            success: 0,
            message: 'Internal server error. Please try again.',
...
app.post('/dashboard',isAuthenticated,isAuthorized(['faculty', 'student']), (req, res) => {
  // Assuming userEmail is available in the request body
  const userEmail = req.body.email;
